...
import { SocketServerPayload, SocketClientPayload } from "../types/SocketActions";
...
export class SocketHelper {
    ...
    constructor(
        readonly socket: SocketConnection['socket'],
        private readonly server: FastifyInstance,
        private readonly cookies: RequestCookie
    ...
    public error(error: SocketError) {
        ...
        return this.socket.send(JSON.stringify(error.payload));
    }
    ...
    private async message(data: WebSocket.RawData) {
        ...
        switch(message.op) {
            case 'PLACE': this.place(message); break;
            default: {
                this.error(new UnknownOperationError());
    ...
    private async place(message: SocketClientPayload<'PLACE'>) {
        ...
        if(!pixel) return this.error(new IncorrectPixelError(message.id ?? Math.random().toString(8).slice(2)));
